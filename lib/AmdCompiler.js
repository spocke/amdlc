...
function parseModuleFile(options) {
	...
	function resolveIdToPath(id) {
		if (options.rootNS) {
			id = id.replace(options.rootNS.replace(/\.$/, '') + '.', '');
		}

		id = id.replace(/\./g, '/');

		return path.join(options.baseDir, id + ".js");
	}
				...
				if (call[1][1] == "define") {
					...
					if (args[1][0] == "array") {
						args[1][1].forEach(function(arg) {
							...
							// Load dependecy
							if (depId.indexOf(options.rootNS) !== -1) {
								processModuleFile(resolveIdToPath(depId));
							}
						});
					}

					// Check that the dependency array is equal to the number of arguments to the define callback
					if (args[2][0] == "function" && args[1][1].length !== args[2][2].length) {
						throw new Error("Dependecy array not equal to define callback arguments in file: " + filePath);
					}
					...
					if (args[0][0] == "string") {
						...
						// Inject global
						if (options.globalModules && options.globalModules[id]) {
							stack[stack.length - 3].push(['var', [[options.globalModules[id], ['sub', ['name', 'modules'], ['name', idVarName]]]]]);
						}
					}
	...
	if (options.from instanceof Array) {
		...
		if (options.from.indexOf('*') != -1) {
			...
			require("glob").sync(options.from).forEach(processModuleFile);
		} else {
			// Parse single file
			processModuleFile(options.from);
		}
	}
...
function writeSourceVersion(parserData, options) {
	var source = "";
	var outFile = options.outputSource, lastMod = 0;

	parserData.modules.forEach(function(module) {
		lastMod = Math.max(getFileModTime(module.filePath), lastMod);
	});

	if (options.force === true || getFileModTime(outFile) !== lastMod) {
		...
		setFileModTime(outFile, lastMod);
	}
}

// Writes and inline minified version of the parsed modules to the outFile
function writeCompressedVersion(parserData, options) {
	var ast;
	var outFile = options.outputMinified, lastMod = 0;
	...
	if (options.force === true || getFileModTime(outFile) !== lastMod) {
		...
		var source = pro.gen_code(ast, {
			ascii_only: true,
			beautify: options.compress === false
		});
...
function writeDevelopmentVersion(parserData, options) {
	var source = "";
	var outFile = options.outputDev, lastMod = 0;
...
function compile(options) {
	...
	var parserData = parseModuleFile(options);

	if (options.outputSource) {
		writeSourceVersion(parserData, options);
	}

	if (options.outputMinified) {
		writeCompressedVersion(parserData, options);
	}

	if (options.outputDev) {
		writeDevelopmentVersion(parserData, options);
	}
}
