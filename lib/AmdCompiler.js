...
function compileSource(modules, instrument) {
	...
	if (_options.outputSource) {
		var source = "";
		var outFile = _options.outputSource;
		...
		currentReporter.info("Writing source version output to: " + utils.toUnixPath(_options.outputSource));

		if (_options.version && _options.releaseDate) {
			source = "// " + _options.version + " (" + (_options.releaseDate) + ")\n\n" + source;
		}

		fs.writeFileSync(utils.createDirForFile(_options.outputSource), source);
	}
}
	...
	try {
		...
		if (_options.force || currentHash != previousHash) {
			...
			if (_options.outputCoverage) {
				compileCoverage(modules, _options);
			}
		}
	} catch (ex) {
