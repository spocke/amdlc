		...
		vm.runInNewContext(source, {
			define: function(id, deps, func) {
				...
				deps.forEach(function(id) {
					var path = resolveIdToPath(id);

					if (id.indexOf(options.rootNS) === 0 && !resolvedPaths[path]) {
						parseModule(path);
					}
				});
			}
...
function mangleModuleIds(body) {
	var ast, value, args, elements, modules = [];
		...
		if (ast instanceof UglifyJS.AST_Call && ast.expression.name === "define") {
			...
			if (args.length == 3) {
				if (args[0] instanceof UglifyJS.AST_String) {
					value = idToVarName(args[0].value);
	...
	body.unshift(new UglifyJS.AST_SimpleStatement({
		body: new UglifyJS.AST_Var({
...
function compileMinified(modules, options) {
	...
	if (options.lastMod != getFileModTime(options.outputMinified)) {
		...
		loader = UglifyJS.parse("" + fs.readFileSync(path.join(__dirname, "AmdInlineLoader.js")));
		innerScope = loader.body[0].body.expression.body;
		innerScope[innerScope.length - 1] = toplevel;
		toplevel = loader;
		toplevel.figure_out_scope();
		...
		source = toplevel.print_to_string({});
		...
		setFileModTime(options.outputMinified, options.lastMod);
	}
}
...
function compileSource(modules, options) {
	if (options.lastMod != getFileModTime(options.outputSource)) {
		...
		setFileModTime(options.outputSource, options.lastMod);
	}
}
...
function compileDevelopment(modules, options) {
	...
	if (options.lastMod != getFileModTime(options.outputDev)) {
		...
		setFileModTime(options.outputDev, options.lastMod);
	}
}
