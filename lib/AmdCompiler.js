...
function parseModules(options) {
	...
	function parseModule(filePath) {
		...
		if (options.version) {
			...
			source = source.replace(/@@majorVersion@@/g, options.version.split('.')[0]);
			source = source.replace(/@@minorVersion@@/g, options.version.split('.')[1]);
		}
...
function generateHash(modules, options) {
	...
	modules.forEach(function(module) {
		...
		hashData += getFileModTime(module.filePath);
	});
...
function parseHash(scriptFile) {
	if (fs.existsSync(scriptFile)) {
		var matches = /\$hash: ([a-z0-9]+)$/.exec(getFileContents(scriptFile));
