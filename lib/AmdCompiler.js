...
function parseModules(options) {
	...
	function resolveIdToPath(id) {
		...
		if (/^[^\/\\]/.test(id)) { // do not resolve if id begins with slash
			id = path.join(options.baseDir, id + ".js");
		}

		return toUnixPath(id);
	}
...
function mangleModuleIds(body) {
	var ast, value, id, varName, args, elements, moduleIdToVars = {};
		...
		if (ast instanceof UglifyJS.AST_Call && ast.expression.name === "define") {
			...
			if (args.length == 3) {
				if (args[0] instanceof UglifyJS.AST_String) {
					id = args[0].value;
					varName = idToVarName(id);

					args[0] = new UglifyJS.AST_SymbolVar({
						name: varName
					});

					moduleIdToVars[id] = varName;
				}
