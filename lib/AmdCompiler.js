...
function compileMinified(modules, options) {
	...
	if (options.outputMinified && options.lastMod != getFileModTime(options.outputMinified)) {
		modules.forEach(function(module) {
...
function compileSource(modules, options) {
	...
	if (options.outputSource && options.lastMod != getFileModTime(options.outputSource)) {
		var source = "";
...
function compileDevelopment(modules, options) {
	...
	if (options.outputDev && options.lastMod != getFileModTime(options.outputDev)) {
		// Add expose call
		var publicModules = getPublicModules(modules);
		if (publicModules.length > 0) {
			source += "\n\texpose([";
